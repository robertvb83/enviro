...
def send_pump_pulse(pump_index: int, turn_on: bool):
    ...
    6 pulses: pump 2 off
    """
    pulse_count = pump_index * 2 + (1 if turn_on else 2)
    print(f"\n--- Sending {pulse_count} pulse(s) for Pump {pump_index} {'ON' if turn_on else 'OFF'} ---")

    short = 0.02  # 20ms pulse
    gap = 0.1     # 100ms between pulses
